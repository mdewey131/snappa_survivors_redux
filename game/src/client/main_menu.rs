...
use crate::{
    render::ui::{button::ButtonSystems, screen_transition::ScreenTransition},
...
fn start_game(
    ...
    let client_ent = *q_client;
    commands.trigger(Connect { entity: client_ent });
...
fn spawn_main_menu(mut commands: Commands, assets: Res<AssetServer>, systems: Res<ButtonSystems>) {
    ...
    let start_game_sys = systems.get("start_game").unwrap();
    let open_settings_sys = systems.get("open_settings").unwrap();

    let start_button = GameButton::new(GameButtonOnRelease::TriggerSystem(*start_game_sys));
    ...
    let st_btn_ent = start_button.spawn(&mut commands, &assets, start_button_style);

    commands
        .entity(st_btn_ent)
        .insert((StartGameButton, ChildOf(button_well)));
